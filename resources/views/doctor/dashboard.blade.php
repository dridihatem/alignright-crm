...
@endpush
 ...
 <div class="container-xxl flex-grow-1 container-p-y">
  @include('partials.dashboard_kpis', ['kpis' => [
      ...
      ['label' => __('master.count_patient'),'value' => $count_patient, 'icon' => 'tabler-users',       'color' => 'info'],
      ['label' => __('master.cases'),        'value' => $count_cases,   'icon' => 'tabler-files',       'color' => 'success'],
  ...
  <div class="row g-4 mb-4">
    ...
    <a href="{{ route('doctor.cases.create') }}" class="btn btn-primary waves-effect waves-light"> <i class="icon-base ti tabler-plus"></i> {{ __('master.create_case') }}</a>
  </div>

        </div>
        <div class="card-body">
          @include('partials.cases_by_patient', ['patientGroups' => $patientGroups, 'caseShowRoute' => 'doctor.cases.show', 'tableId' => 'doctorDashboardPatients'])
        </div>
      </div>
    </div>
  </div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
      var options = {
          ...
          colors: ['#8592a3', '#ffab00', '#00bad1', '#03c3ec', '#71dd37'],
          xaxis: {
              categories: [
                  '{{ __("master.draft") }}',
                  '{{ __("master.pending") }}',
