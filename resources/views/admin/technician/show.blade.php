<x-app-layout>
    <x-slot name="title">{{ __('master.technician_details') }} - {{ __('master.admin') }}</x-slot>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{ __('master.technician_details') }}</li>
            </ol>
        </nav>
            ...
            <div class="card-header d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-0">
                        ...
                        {{ __('master.technician_details') }}
                    </h5>
                    <small class="text-muted">{{ __('master.technician_information') }}</small>
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('admin.technicians.edit', $technician->id) }}" class="btn btn-primary btn-sm">
                        <i class="fas fa-edit me-1"></i> {{ __('master.edit') }}
                    </a>
                    <a href="{{ route('admin.technicians.list') }}" class="btn btn-secondary btn-sm">
            ...
            <div class="col-md-4">
                <div class="card mb-4">
                    <div class="card-header">
                        <h6 class="mb-0">{{ __('master.technician_information') }}</h6>
                    </div>
                    <div class="card-body text-center">
                        <img src="{{ $technician->photo_url }}" 
                             alt="{{ $technician->name }}" 
                        ...
                        <p class="text-muted mb-3">{{ $technician->email }}</p>
                        
                        @if($technician->status == 'active')
                            <span class="badge bg-label-success mb-3">{{ __('master.active') }}</span>
                        @else
                            <span class="badge bg-label-danger mb-3">{{ __('master.inactive') }}</span>
                        @endif

                        <div class="row text-start">
                            @if($technician->phone)
                                <div class="col-12 mb-2">
                                    <strong>{{ __('master.phone') }}:</strong> {{ $technician->phone }}
                                </div>
                            @endif
                            
                            @if($technician->address)
                                <div class="col-12 mb-2">
                                    <strong>{{ __('master.address') }}:</strong> {{ $technician->address }}
                                </div>
                            @endif
                            
                            @if($technician->specialization)
                                <div class="col-12 mb-2">
                                    <strong>{{ __('master.specialization') }}:</strong> {{ $technician->specialization }}
                                </div>
                            @endif
                            
                            @if($technician->license_number)
                                <div class="col-12 mb-2">
                                    <strong>{{ __('master.license_number') }}:</strong> {{ $technician->license_number }}
                                </div>
                            @endif
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row text-center">
                            ...
                            <div class="col-6">
                                <h4 class="mb-1 text-success">{{ $cases->where('status', 'in_production')->count() }}</h4>
                                <small class="text-muted">{{ __('master.in_production') }}</small>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        @if($cases->count() > 0)
                            <div class="table-responsive">
                                <table class="table table-hover technician-cases-table">
                                    <thead>
                                        <tr>
    ...
    <script>
        $(document).ready(function() {
            // Initialize DataTable for technician cases
            $('.technician-cases-table').DataTable({
                processing: true,
                serverSide: false,
                pageLength: 10,
                responsive: true,
